...
| GET | `/finance/reports/forecast` | sanctum + verified | SpendingForecastReport. Sin parámetros. Proyección de gasto del mes en curso por categoría usando método lineal por ritmo (`gastado × dias_mes / dias_transcurridos`) comparado contra promedio de los 3 meses calendario inmediatamente anteriores. Sólo incluye categorías con al menos 1 entry de gasto en la ventana histórica. |
...
| GET | `/finance/reports/forecast/export.xlsx` | sanctum + verified | Export Excel de proyección de gasto del mes en curso (sin params). |
...
- **Filosofía libreta libre en el carril simulado, con auto-ajuste de pagos a tarjeta**: los saldos cash/débito pueden cruzar 0 sin error; los cargos a tarjeta pueden exceder `credit_limit`. Pero los `debt_payment` planeados **nunca dejan la tarjeta en negativo**: si el monto excede la deuda actual, se recorta al monto exacto adeudado (`warning = "auto_adjusted"`); si la deuda ya está en 0 al momento de la ocurrencia, se salta entera (`warning = "debt_already_zero"`). La única validación dura al crear movimientos reales sigue siendo `OverpayDebt` en `PayCreditAccount`.
