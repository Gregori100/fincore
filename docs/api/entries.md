...
|-----------|------|---------|-------------|
| `account_id` | int | — | Filtra entries donde la cuenta sea origen O destino |
...
Las relaciones `origin` y `destination` (que apuntan a la cuenta) vienen cargadas automáticamente para evitar N+1. Si una de las dos es `null` (caso de `income` o `expense` puro), el campo respectivo es `null` en el JSON.
