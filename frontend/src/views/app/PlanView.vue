...
import { PlusIcon, TrashIcon, QueueListIcon } from '@heroicons/vue/24/outline'
...
import PlannedEventBulkForm from '@/components/finance/PlannedEventBulkForm.vue'
...
const openModal = ref(null) // 'event' | 'override' | 'bulk' | null
...
<template>
  <AppLayout>
    <div class="space-y-6">
      <header class="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div class="flex items-center gap-2">
          ...
          <BaseButton
            variant="secondary"
            :disabled="!auth.isVerified"
            title="Agregar varios eventos de golpe"
            @click="openModal = 'bulk'"
          >
      ...
      <!-- Dos columnas: proyección (izquierda, fija visible) + eventos (derecha,
           panel con scroll propio). En móvil se apilan, proyección primero. -->
      <div class="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-6 items-start">
        ...
        <section class="min-w-0">
          <div class="flex items-center justify-between gap-2 mb-3">
            <h3 class="text-sm font-semibold uppercase tracking-wide text-[color:var(--color-text-subtle)]">
              Eventos planeados
              <span v-if="plan.events.length" class="text-[color:var(--color-text-subtle)] normal-case font-normal">
                · {{ plan.events.length }}
              </span>
            </h3>
          </div>
    ...
    <BaseModal :open="openModal === 'bulk'" title="Carga masiva de eventos" size="4xl" persistent @close="close">
      <PlannedEventBulkForm @close="close" @success="close" />
    </BaseModal>
