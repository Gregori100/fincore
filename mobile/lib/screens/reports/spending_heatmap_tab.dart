...
/// Tab "Heatmap" (10mo) de /reports. Sprint
/// `flutter-reports-spending-heatmap-v1`.
///
/// Muestra un año completo de gastos como **12 mini-heatmaps mensuales**
/// dispuestos en grilla 3×4. Cada mini es una cuadrícula 7 columnas
/// (Lun–Dom) × 6 filas (semanas del mes) con celdas coloreadas según la
/// intensidad de gasto por día (RN-HM06). Rediseño post-smoke con Diego:
/// el layout año-completo estilo GitHub quedaba con celdas de ~5 px en
/// cel de 360 px, difícil de leer. El grid 3×4 da ~12 px por celda,
/// preservando la vista anual con legibilidad.
///
/// Tap en cualquier celda con gasto abre `/entries` con filtro custom
/// (`from=to=día`, `kinds=['expense','credit_expense']`, RN-HM09).
class SpendingHeatmapTab extends StatefulWidget {
  const SpendingHeatmapTab({super.key});

  @override
  State<SpendingHeatmapTab> createState() => _SpendingHeatmapTabState();
}

class _SpendingHeatmapTabState extends State<SpendingHeatmapTab> {
  late int _focusedYear;
  Stream<SpendingHeatmap>? _stream;
  ...
  Stream<SpendingHeatmap> _buildStream() {
    final deps = AppDependencies.of(context);
    return deps.reportsService.spendingHeatmap(year: _focusedYear);
  }
  ...
  // Cache del último SpendingHeatmap recibido, para pasarlo al sheet
  // sin depender del snapshot en el momento del tap.
  SpendingHeatmap? _lastHeatmap;

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<SpendingHeatmap>(
      stream: _stream,
...
/// Retorna el `DateTime` correspondiente a la celda `(column, row)` de
/// un mini-heatmap mensual, donde column 0 = Lunes, ..., column 6 = Dom.
/// La primera columna de la primera fila corresponde al Lunes de la
/// semana en que cae el día 1 del mes; los días previos al 1 quedan como
/// spillover (retorna null).
DateTime? heatmapDayForMonthPosition(int year, int month, int column, int row) {
  final firstDay = DateTime(year, month, 1);
  final firstWeekday = firstDay.weekday; // 1=Mon..7=Sun
  final dayNumber = row * 7 + column - (firstWeekday - 1) + 1;
  if (dayNumber < 1) return null;
  final result = DateTime(year, month, dayNumber);
  // Si `dayNumber` excede los días del mes, DateTime rueda al siguiente
  // (ej: 32 de enero → 1 de febrero). Filtramos ese spillover.
...
Color _colorForIntensity(IntensityLevel level) {
  switch (level) {
    ...
    case IntensityLevel.low:
      return FincoreColors.negative.withValues(alpha: 0.25);
    case IntensityLevel.medium:
      return FincoreColors.negative.withValues(alpha: 0.45);
    case IntensityLevel.high:
      return FincoreColors.negative.withValues(alpha: 0.7);
    ...
    return Container(
      ...
      child: const Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Sin gastos registrados en este año.',
              style: TextStyle(
                color: FincoreColors.accent,
                fontSize: 12,
              ),
